...
```{r null_tar_del_profile, eval=FALSE, fig.width=7.5, fig.height=2.15}
...
null_del_profile$count <- NULL

null_del_profile <- split(null_del_profile, null_del_profile$specimen)

null <- lapply(seq_along(null_del_profile), function(i, target, totals, fp){
  
    p <- plot_indel_cov(null_del_profile[[i]], target, totals = totals) + 
      custom_theme
    
    file_pdf <- sprintf("null_target_del_profile-%s.pdf", i)
    file_png <- sprintf("null_target_del_profile-%s.png", i)
...
if( args$format == "pdf"){
  ...
  knitr::include_graphics(
    path = sprintf(
      file.path(figure_path, "null_target_del_profile-%s.pdf"), 
      seq_along(null_del_profile)
    )
  )
  ...
  knitr::include_graphics(
    path = sprintf(
      file.path(figure_path, "null_target_del_profile-%s.png"), 
      seq_along(null_del_profile)
    )
  )
